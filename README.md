...
> **Interactive map (HTML):** the committed sample is the **smoke-AOI** map,
> [`docs/figures/exposure_map_smoke.html`](docs/figures/exposure_map_smoke.html)
> (1 km × 1 km tile, 14 assets) — open locally in a browser; GitHub does not
> render HTML inline. The full pilot-AOI map (`docs/figures/exposure_map.html`,
> ≈14 MB — above this repo's 2 MB committed-file cap, so not committed) is
> regenerated by `uv run python scripts/12_make_figures.py` once the pilot
> outputs exist. Layers: assets coloured by rank (popup with class, rank,
> top-3 features), fuel-class overlay, ICNF burn perimeters.
