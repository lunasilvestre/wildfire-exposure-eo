...
> — a layer-centric, thematic browser (**Inputs / Interim / Output / Validation**
> tabs, fly-to bookmarks, a process panel) over a full-Iberia extent, pure
> static (MapLibre, client-side COG rendering): the scored pilot AOI
> (exposure-ranked assets; **Output** shows `impact_severity = exposure_rank ×
> criticality`), full-Iberia **Inputs** (EFFIS fuel, Cop-DEM slope, ETH canopy
> height), burn-scar inference, ICNF + EFFIS burn history, a FireScope
> cross-comparison, and download links for every published artefact.
> `exposure_rank` stays **AOI-relative** — a rank, not a probability. Source:
> [`docs/index.html`](docs/index.html); the pilot data bundle is generated by
> `scripts/15_make_geobrowser_data.py`, the full-Iberia display/validation
> layers by [`scripts/29_make_iberia_inputs.py`](scripts/29_make_iberia_inputs.py)
> and [`scripts/30_make_iberia_burn_history.py`](scripts/30_make_iberia_burn_history.py).
...
*Attribution: OSM contributors · ESA/Copernicus · ICNF · EFFIS/CEMS · DGT · ETH GCH (Lang et al. 2023) · FireScope (INSAIT/ETH, CC-BY-4.0)*
...
| EFFIS European Fuel Map | JRC / Copernicus | GeoTIFF (~250 m, EPSG:3035), EU open data | **REFERENCE** — international NFFL-13 crosswalk anchor; full-Iberia fuel display layer |
| Scott & Burgan FBFM40 | USFS / LANDFIRE | reference document | International fuel-model framework |
| ICNF Áreas Ardidas | ICNF (Instituto da Conservação da Natureza e Florestas) | annual polygons (Shapefile/GPKG) | Validation ground truth, history feature; PT side of Iberia burn history (1990–2025) |
...
| EFFIS Burned Area | JRC / Copernicus (WFS 1.0.0) | annual polygons (2016–2025) | Cross-border validation; ES side of Iberia burn history |
| FireScope `oracle_unet` | INSAIT Institute + ETH Zürich | GeoTIFF (~30 m, EPSG:3857), CC-BY-4.0 | **VALIDATION** — Europe-wide relative wildfire-risk **rank** (arXiv:2511.17171); cross-comparison only, never a probability |
...
**Full-Iberia geobrowser layers.** The thematic geobrowser also renders peninsula-scale display/validation derivatives over the Iberia bbox `(-9.8, 35.9, 3.5, 44.0)`: EFFIS fuel class (~250 m, NEAREST), Cop-DEM GLO-30 slope (computed in EPSG:3035 for cross-zone accuracy, displayed at 40 m), ETH canopy height (10 m → ~30 m), the FireScope risk-rank clip (Validation), and the ICNF + EFFIS burn-history overlay. These are a faithful reproducible record in [`scripts/29_make_iberia_inputs.py`](scripts/29_make_iberia_inputs.py) and [`scripts/30_make_iberia_burn_history.py`](scripts/30_make_iberia_burn_history.py); each layer's full source, license, resolution, CRS, and known limits are in [`docs/data_sources.md`](docs/data_sources.md) → *Full-Iberia geobrowser layers*.
...
[FireScope](https://arxiv.org/abs/2511.17171) (INSAIT Institute + ETH Zürich)
publishes a public Europe-wide wildfire *risk* map ("risk" is their vocabulary,
and its uint8 units are undocumented, so we treat it strictly as a relative
*rank* — everything in this repo stays *exposure rank*, never a probability),
released CC-BY-4.0 on Hugging Face (`INSAIT-Institute/firescope-risk-2026`,
revision `c387af415530`). A head-to-head comparison of this repo's transparent
screen against FireScope is in
[`docs/firescope_benchmark.md`](docs/firescope_benchmark.md)
([`scripts/28_firescope_benchmark.py`](scripts/28_firescope_benchmark.py)), and
an Iberia clip is rendered as the geobrowser's **Validation** layer
([`scripts/29_make_iberia_inputs.py`](scripts/29_make_iberia_inputs.py)).
Commercial utility vegetation-management products mirror the operational shape
of this pipeline with proprietary imagery and ground truth; this repo is the
open-data, open-method counterpart.
