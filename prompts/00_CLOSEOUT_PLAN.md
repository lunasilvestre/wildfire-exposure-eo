...
| **WU-9** | `15_geobrowser.md` | Publish geographic outputs at full fidelity (scored GeoParquet + fuel COG as committed STAC assets; 36 MB burn-scar COG → GitHub Release) + a pure-static **GitHub Pages geobrowser** (MapLibre, client-side full-fidelity COG/vector — no downsampling) presenting study + inputs + outputs + **Mermaid** pipeline/reproduction/lineage diagrams | PUBLIC SURFACE: rank not probability (#6), no production claims (#9), explicit CRS (#2). **Fable build** (`CLOSEOUT_MODEL_DEFAULT=fable`). Headless build can't visually verify the map — Nelson checks the live Pages URL; orchestrator enables Pages via `gh api` after the review is green. Run from a fresh orchestrator (`prompts/_HANDOVER_WU9.md`) |
