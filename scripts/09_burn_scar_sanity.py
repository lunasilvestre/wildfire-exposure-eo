...
def fetch_perimeters_in_bbox(
    bbox: tuple[float, float, float, float], *, layer_id: int
) -> list[dict]:
    """All ICNF features (GeoJSON, WGS84) intersecting `bbox`, paged.

    The truth mask must be the union of *every* mapped burn in the inference
    box, not just the target polygon — neighbouring same-season fires would
    otherwise count as false positives and depress precision/IoU.
    """
    features: list[dict] = []
    offset = 0
    page_size = 200
    while True:
        resp = requests.get(
            ...
            params={
                ...
                "outFields": "OBJECTID,DH_Inicio,AreaHaPoly",
                "returnGeometry": "true",
                "outSR": "4326",
...
def check_one(feature: dict, handle: burn_scar.ModelHandle, cfg: Any, *, layer_id: int) -> dict:
    """Run inference around one perimeter; return agreement metrics.

    Truth = union of all mapped burns in the box whose start date falls
    inside the S2 window (not just the target polygon).
    """
    ...
    bounds = aoi.bounds
    in_box = fetch_perimeters_in_bbox(
        (bounds[0], bounds[1], bounds[2], bounds[3]), layer_id=layer_id
    )
    truth_features = []
    for f in in_box:
        raw = f.get("properties", {}).get("DH_Inicio")
        if raw is None:
            continue
        f_started = datetime.fromtimestamp(raw / 1000.0, tz=UTC).date()
        if window_start <= f_started <= window_end:
            truth_features.append(f)
    print(
        f"[sanity]   truth mask: {len(truth_features)} perimeter(s) in box+window "
        f"(target OBJECTID {props['OBJECTID']})",
        file=sys.stderr,
    )
    ...
    mask = rasterize(
        [(f["geometry"], 1) for f in truth_features],
    ...
    return {
        ...
        "truth_polys": len(truth_features),
        "scenes": len(items),
...
def main() -> int:
    ...
    for objectid in args.objectids:
        ...
        rows.append(check_one(feature, handle, cfg, layer_id=args.layer_id))

    commit = code_commit_sha(cwd=Path.cwd())
    print(f"<!-- generated by: scripts/09_burn_scar_sanity.py at {commit} -->")
    print()
    print(
        "| OBJECTID | concelho | area (ha) | fire | S2 window | truth polys | scenes "
    ...
    for r in rows:
        print(
            ...
            f"| {r['window']} | {r['truth_polys']} | {r['scenes']} | {r['iou']:.3f} "
            f"| {r['precision']:.3f} | {r['recall']:.3f} "
            f"| {r['mean_prob_inside']:.3f} / {r['mean_prob_outside']:.3f} |"
        )
    return 0
