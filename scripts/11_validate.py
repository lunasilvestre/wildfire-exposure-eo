...
def _metrics_block(title: str, m: dict[str, Any], features: list[str]) -> list[str]:
    ...
    n_deciles = len(m["table"])
    if m["n_burned"] < n_deciles:
        quantum = 1.0 / (int(m["table"].iloc[0]["n_assets"]) * m["base_rate"])
        lines += [
            f"With only {m['n_burned']} burned asset(s) across {n_deciles} deciles, "
            f"per-decile lift is quantised in steps of ≈{quantum:.2f}× — read the "
            "cumulative column, not any single decile.",
            "",
        ]
    return lines
...
def main() -> int:
    ...
    emit_sha = code_commit_sha(cwd=Path.cwd())
    lines: list[str] = [
        "# Exposure-rank validation against subsequent ICNF burns",
        "",
        f"<!-- generated by: scripts/11_validate.py at {emit_sha} ; "
        f"input {exposure_path.name} scored at {commit} -->",
        "",
        ...
        f"after the window end — vintage year(s) **{validation_years or 'none'}**: "
        f"{len(validation_burns)} perimeter(s), "
        f"{float(validation_burns['area_ha'].sum()):.0f} ha inside the AOI. The "
    ...
    if m_full["degenerate"] or m_ablation["degenerate"]:
        ...
        # features carry the signal — saying so beats a noise-driven verdict.
        top_n = int(m_full["table"].iloc[0]["n_assets"])
        lift_quantum = 1.0 / (top_n * m_full["base_rate"])
        delta_txt = (
            ...
            f"({m_full['spearman_rho']:.4f} → {m_ablation['spearman_rho']:.4f})."
        )
        if abs(d_lift) <= lift_quantum:
            carries_txt = (
                f"With {m_full['n_burned']} burned asset(s), a single asset moving "
                f"in or out of the top decile changes its lift by {lift_quantum:.2f}× — "
                f"the observed difference is within that one-asset granularity, so this "
                f"run **does not resolve** which features carry the signal. Re-run as "
